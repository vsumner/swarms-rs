...
fn test_suite_discovery() {
    ...
    for test_file in test_files {
        let content =
            std::fs::read_to_string(&test_file).expect("Should be able to read test file");
...
fn test_workspace_configuration() {
    ...
    let content = std::fs::read_to_string(&cargo_toml).expect("Should be able to read Cargo.toml");
