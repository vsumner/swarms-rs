...
impl Config {
    fn from_env() -> Result<Self> {
        Ok(Config {
            ...
            model: "gpt-4".to_string(), // Using a standard model name
        })
    }
}
...
async fn main() -> Result<()> {
    ...
    let client = OpenAI::from_url("https://api.openai.com/v1", &config.api_key)
    ...
    let sequential_config = SwarmRouterConfig {
        ...
        rules: Some(
            "1. Each agent must maintain professional tone\n\
             ...
             3. Reports must be clear and actionable".to_string(),
        ),
    ...
    let concurrent_config = SwarmRouterConfig {
        ...
        rules: Some(
            "1. All analysis must be data-driven\n\
             ...
             3. Provide clear risk levels and confidence scores".to_string(),
        ),
        multi_agent_collab_prompt: true,
    };
